...
// helper module so string doesn't have to be exact
...
const cleanWord = word =>word.trim().toLocaleLowerCase().replace(/[^a-z]/gi, '')
...
export default function Teleprompter({words,progress,listening,onChange}) {
  ...
  React.useEffect(() => {
    const handleResult = ({results}) => {
      const interim = Array.from(results).filter(r => !r.isFinal).map(r => r[0].transcript).join(' ');
      setResults(interim);
      ...
      const newIndex = interim.split(' ').reduce((memo, word) => {
          ...
          const similarity = stringSimilarity.compareTwoStrings(cleanWord(word),cleanWord(words[memo]));
  ...
  return (
    <React.Fragment>
      <StyledTeleprompter ref={scrollRef}>
        {words.map((word, i) => (
            <span key={`${word}:${i}`} data-index={i} style={{color:i < progress ? '#ccc' : '#000'}}>
                {word}{' '}
            </span>
        ))}
      </StyledTeleprompter>
